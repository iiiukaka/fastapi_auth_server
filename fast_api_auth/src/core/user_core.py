...
def get_refresh_jwt_strategy() -> JWTStrategy:
    ...
    return JWTStrategy(
        ...
        lifetime_seconds=project_settings.jwt_refresh_lifetime_seconds
    )
...
class UserManager(UUIDIDMixin, BaseUserManager[User, UUID]):
        ...
        if len(password) < project_settings.min_password_length:
            raise InvalidPasswordException(
                reason=(
                    ...
                    f'{project_settings.min_password_length} символов'
                )
            )
    ...
    async def on_after_login(
        ...
        refresh_token = await refresh_auth_backend.get_strategy().write_token(
            user
        )
        response.set_cookie(
